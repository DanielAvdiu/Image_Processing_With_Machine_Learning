...
if __name__=="__main__":
    create_dir("results_pruned_more")

    """ Load Model """
    loaded_model = tf.keras.models.load_model(
        "files_pruned_more/model_pruned_more.h5",
        custom_objects={
    ...
    # Strip the pruning wrappers from the loaded model
    model = tfmot.sparsity.keras.strip_pruning(loaded_model)
    ...
    for x, y in tqdm(zip(test_x, test_y), total=len(test_x)):
        ...
        save_path=f"./results_pruned_more/{name}"
    ...
    df.to_csv("files_pruned_more/score_pruned_more.csv")